...
from petitlink.settings import AuthSettings, auth_settings
...
def patch_settings(request: FixtureRequest) -> Iterator[AuthSettings]:
    ...
    for k, v in env_vars_to_patch.items():
        ...
        if not hasattr(auth_settings, k):
            raise ValueError(f'Unknown setting: {k}')

        # Raise an error if the env var has an invalid type
        expected_type = getattr(auth_settings, k).__class__
        if not isinstance(v, expected_type):
            raise ValueError(
                f'Invalid type for {k}: {v.__class__} instead '
                f'of {expected_type}'
            )
        setattr(auth_settings, k, v)

    yield auth_settings

    # Restore the original settings
    auth_settings.__dict__.update(original_settings.__dict__)
